...
1. **3a 를 직접 플레이해서 확인한다** — `게임 열기.cmd` → **F5**. 코드로는 판단이 안 된다.
   - **Tab 으로 제작창을 열어 데미지·범위 슬라이더를 바꾸고 저장** → 나가서 Q 로 쏴본다.
     수치를 바꾼 게 손맛으로 느껴지는가? 세면 답답하고 약하면 시원한, 그 교환이 재미있는가?
   - **이동/점프 감각** (멧챠 카멜레온풍인가? `move_speed 6.0` / `jump_height 1.2`)
   - **쿨타임 체감** — 불꽃탄 기준 4.94초. 예산 공식이 뱉은 값 그대로라
     개발이 임의로 줄이지 않았다. 길면 기획 담당이 `Balance.gd` 를 조정한다.
2. **예산 초과 스킬을 어떻게 할지 정한다** (아래 「막힌 것」 참고). 3b 가기 전에 정리돼야 한다.
3. 재미가 확인되면 **3b(그림판 + 형태 태그)** 로 넘어간다.
   그림이 겉모습이 되고 태그별 히트박스 4종(캡슐·원·부채꼴·산탄)이 붙는 단계다.
   **3a 가 재미있다고 확인되기 전에는 손대지 않는다** — 기획서 87~88행.

## ✅ 완료
- **🎚 로드맵 3a: 에디터 수치가 실제 발사에 반영** (2026-07-31)
  - `Player.gd` 가 `SkillDB.get_slot("Q")` 에서 이름·데미지·범위·색을 읽는다.
    하드코딩 값은 `fallback_*` 로 남아 슬롯이 빈 첫 실행에만 쓰인다
  - `SkillDB.slot_changed` 구독 → 제작창에서 **저장하면 다음 발사부터 바로 반영**
  - **그림(mask)은 아직 안 읽는다.** 형태 태그는 항상 `둥긂` 고정 — 그림 연결은 3b
  - 제작창 표시를 사실대로 고침: 전투 수치를 `둥긂` 기준으로 계산(= 실제로 나가는 것과 일치),
    태그는 `형태: 뾰족함 (3b 예정 — 지금은 둥긂으로 나감)` 으로 명시.
    안 그러면 화면엔 부채꼴이 뜨는데 실제로는 구가 날아간다 — 지어낸 값이 된다
  - 검증: 슬롯을 5/4 로 바꾸니 쿨 4.94→2.28초, 반경 0.95→0.63m, 명중 데미지 22→5 로 즉시 반영
...
## ⛔ 막힌 것 / 결정 대기
- **예산 초과 스킬이 그대로 저장·발사된다** (2026-07-31, 기획 담당 결정 필요)
  - 제작창은 초과 시 빨간 글씨로 "초과!"만 띄우고 막지는 않는다
  - 데미지 100 / 범위 100 = 비용 160pt 인데 `spent_ratio` 가 1.0 으로 clamp 돼
    쿨타임이 최대 12초에서 멈춘다 → **100pt 를 지킨 스킬보다 이득이다**
  - 기획서 9행 「형평성 원칙」에 어긋나지만, 막는 규칙이 기획서에 없어
    개발이 임의로 만들지 않았다. 선택지: ①저장 자체를 막기 ②발사를 막기
    ③초과분에 페널티를 주도록 공식 수정 ④그대로 두기
  - 3b 로 넘어가기 전에 정리하는 게 좋다 — 그림까지 붙으면 확인할 게 더 늘어난다
