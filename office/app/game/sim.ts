...
/** 착수 대기 부서가 멈춰 있는 진짜 이유 */
const BLOCK_REASON: Record<string, string> = {
  brand: "밸런스 계수가 아직 없어요. 실제로 돌려보고 정해야 하는 수치라 감으로 만들지 않습니다. 전투가 돌아가면 바로 시작해요.",
  partner: "전투가 아직 없어서 붙일 소리가 없어요. 지금 만들면 버려집니다. 프로토타입 나오면 착수합니다.",
  finance: "빌드할 게 아직 없어요. 로드맵 1단계만 끝나면 그날 첫 exe 뽑아드립니다.",
};

/** 지시창에서 부서를 찾을 때 쓰는 키워드 — 구체적인 것부터 검사한다 */
const DEPT_KEYWORDS: [string, string[]][] = [
  ["qa", ["qa", "큐아", "검수", "버그", "악용", "윤규아"]],
  ["brand", ["밸런스", "수치", "예산", "계수", "박보라"]],
  ["strategy1", ["기획", "기획팀", "아이디어", "스킬 컨셉", "최아름", "톱3", "top 3"]],
  ["strategy2", ["설계", "시스템", "스펙", "데이터 구조", "한도빈"]],
  ["research", ["레퍼런스", "리서치", "조사팀", "경쟁작", "참고", "김서연"]],
  ["reels", ["개발", "구현", "godot", "고도", "클라이언트", "코드", "송리원"]],
  ["carousel", ["아트", "이펙트", "그래픽", "리소스", "스프라이트", "이가림"]],
  ["partner", ["사운드", "소리", "효과음", "bgm", "정파랑"]],
  ["finance", ["빌드", "배포", "exe", "패키징", "오재민"]],
  ["review", ["플레이테스트", "테스트", "재미", "강성아"]],
  ["ops", ["pm", "일정", "우선순위", "커밋", "푸시", "git", "안도현"]],
  ["secretary", ["코디", "코디네이터", "status", "김세리", "브리핑"]],
];
...
export class Company {
  ...
  private *dayScript(): Generator<number | (() => boolean), void, void> {
    ...
    this.pushLog("🚪", `07:00 자동 출근을 시작합니다. 개발팀 ${workers.length}명 입장!`, "yellow");
    this.lock(workers);
    ...
    // ② 레퍼런스 조사
    this.phaseIndex = 2;
    yield* this.runDept("research", "장르·경쟁작 레퍼런스 조사", 6.5, "직접 해본 참고작 5개 정리했어요.");

    // ③ 밸런스 검토 — 착수 전이라 라운지로
    this.phaseIndex = 3;
    const bora = this.agentById.get("brand-lead")!;
    this.stand(bora);
    this.say(bora, "전투가 없어서 밸런스 계수를 못 잡아요.", 3);
    this.pushLog("⚖️", "밸런스 수치팀: 실측할 전투가 없어 수치를 만들지 않고 기록만 남김", "lav");
    this.goto(bora, rand(LOUNGE_ROOM.loiter), "휴식");
    this.enqueue(bora, { k: "wait", dur: 4 }, { k: "fn", fn: () => this.say(bora, "프로토타입 나오면 바로 돌립니다.", 2.4) });
    this.sitAtDesk(bora);
    this.pushLog("🔊", "사운드팀·빌드팀: 붙일 전투와 뽑을 빌드가 아직 없어 오늘은 대기합니다.", "lav");
    ...
    yield* this.meeting(
      ...
      [
        ["research-lead", "참고작 5개예요. 전부 직접 플레이해봤어요."],
        ["strategy1-lead", "좋아요. 우리 게임에 맞는 각도 10개로 풀게요."],
        ["qa-lead", "밸런스 악용 여지부터 확인할게요."],
      ],
    );

    // ⑤ 작업 후보 10개
    this.phaseIndex = 4;
    yield* this.runDept("strategy1", "작업 후보 10개 · 우선순위 채점", 7, "10개 뽑았고 로드맵 순서로 정렬했어요.");

    // ⑥ 설계 검수
    this.phaseIndex = 5;
    yield* this.runDept("qa", "구현 가능성·악용 여지 검사", 5.5, "3개 반려, 7개 통과예요.");
    this.pushLog("🛡️", "QA 반려 3건: 로드맵 순서 무시 / 밸런스 악용 여지 / 검증 방법 없음", "lav");

    // ⑦ TOP 3 선정
    this.phaseIndex = 6;
    const areum = this.agentById.get("strategy1-lead")!;
    this.stand(areum);
    this.say(areum, "이번 주 TOP 3 정리했어요.", 3);
    this.pushLog("💡", "게임 기획팀: TOP 3 확정 (1순위 · 캐릭터 이동+점프)", "pink");
    yield 1.8;
    ...
    this.say(approvers[0], "1순위는 '캐릭터 이동 + 점프' 입니다. 로드맵 1단계예요.", 3.4);
    yield 2.4;
    ...
    this.pushLog("✅", "대표 승인 완료 — 1순위 작업 구현을 시작합니다.", "mint");
    yield 1.6;
    ...
    // ⑨ 시스템 설계
    this.phaseIndex = 8;
    yield* this.runDept("strategy2", "데이터 구조·씬 구성 설계", 7, "스펙 확정했어요. 구현 들어가도 됩니다.");

    // ⑩ 구현 인수인계 → 개발·아트 동시 작업
    this.phaseIndex = 9;
    yield* this.deliver("strategy2-lead", "reels", "스펙 넘길게요. 씬 구성까지 적어뒀어요.", "받았어요! 이동부터 붙일게요.");
    yield* this.deliver("strategy2-lead", "carousel", "필요한 리소스 목록이에요.", "프로토타입이니 도형으로 갑니다.");

    this.startDept("reels", "Godot 구현·플레이 테스트", 8);
    this.startDept("carousel", "플레이스홀더 리소스 제작", 8);
    yield () => this.deptStatus.reels === "완료" && this.deptStatus.carousel === "완료";
    this.pushLog("🎮", "기능 구현 완료 · 플레이스홀더 리소스 반영 (기존 씬은 그대로 보존)", "mint");

    // ⑪ 커밋 + 기록
    this.phaseIndex = 10;
    this.startDept("ops", "커밋·푸시·자동 백업 확인", 5);
    this.startDept("review", "직접 플레이·재미 점검", 5);
    yield () => this.deptStatus.ops === "완료" && this.deptStatus.review === "완료";
    this.pushLog("📦", "커밋 완료 · GitHub 원격까지 푸시했습니다", "mint");

    // ⑫ 김비서 브리핑
    this.phaseIndex = 11;
    this.lock([seri]);
    this.stand(seri);
    this.say(seri, "reports 취합했어요. STATUS.md 갱신하고 보고드립니다.", 3);
    ...
    this.pushLog("📋", `코디네이터 최종 브리핑 완료 — 완료 ${stats.done}팀 · 착수 대기 ${stats.blocked}팀`, "pink");
    yield 3;
    this.stand(seri);
    this.sitAtDesk(seri);
    yield this.allFree([seri]);
